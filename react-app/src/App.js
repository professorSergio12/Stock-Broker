...
function App() {
  ...
  const fetchStats = async () => {
    try {
      ...
      // If backend returns valid stats, use them directly
      if (serverStats && serverStats.overall) {
        // If backend doesn't provide totals, enrich using list endpoint's total
      ...
      // Fallback: derive at least totalTrades from the list endpoint (cheap HEAD-like call)
      try {
        const listRes = await tradesAPI.getTrades({
          page: 1,
        ...
        // If even fallback fails, keep empty stats
        setStats({ overall: {} });
      }
    } catch (error) {
